...
public class ManagerSeguridades {
	 ...
	 * @throws Exception
	 */
	public LoginDTO login(String codigo, String clave) throws Exception {
		if (ModelUtil.isEmpty(clave)) {
			mAuditoria.mostrarLog(getClass(), "login", "Debe indicar una clave " + codigo);
			throw new Exception("Debe indicar una clave");
		}

		List<SegUsuario> listaUsuarios = null;
		listaUsuarios = mDAO.findAll(SegUsuario.class);
		...
		if (usuario == null) {
			mAuditoria.mostrarLog(getClass(), "login", "No existe usuario " + codigo);
			throw new Exception("Error en credenciales.");
		}

		if (usuario.getClave().equals(clave)) {
			if (usuario.getActivo() == false) {
				mAuditoria.mostrarLog(getClass(), "login", "Intento de login usuario desactivado " + codigo);
				throw new Exception("El usuario esta desactivado.");
			}
			mAuditoria.mostrarLog(getClass(), "login", "Login exitoso " + codigo);
			// crear DTO:
			LoginDTO loginDTO = new LoginDTO();
			loginDTO.setIdSegUsuario(usuario.getIdSegUsuario());
			loginDTO.setCorreo(usuario.getCorreo());
		...
		mAuditoria.mostrarLog(getClass(), "login", "No coincide la clave " + codigo);
		throw new Exception("Error en credenciales");
	}
